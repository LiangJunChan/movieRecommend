...
class AutoRec():
    ...
    def prepare_model(self):
        ...
        self.input_mask_R = tf.placeholder(dtype=tf.float32, shape=[None, self.num_items], name="input_mask_R") #占位符，没有初始值

        V = tf.get_variable(name="V", initializer=tf.truncated_normal(shape=[self.num_items, self.hidden_neuron],
                                         mean=0, stddev=0.03),dtype=tf.float32) #共享变量，生成正太分布的数据
        ...
        pre_Encoder = tf.matmul(self.input_R,V) + mu #矩阵相乘
        ...
        pre_cost1 = tf.multiply((self.input_R - self.Decoder) , self.input_mask_R) #点乘
    ...
    def make_records(self):
        ...
        recom = self.result_path + "recom.txt"
        movie=self.movies1

        with open("./results/recom.txt", 'w') as r:
            j=1
            for i in Es[-1]:
                #print(movie["1"])
                r.write(str(i)+"\t"+"%s" %(movie["%d" %j] ) +"\t"+"%d"%j +"\n")
                j=j+1
                if j==1683:break
